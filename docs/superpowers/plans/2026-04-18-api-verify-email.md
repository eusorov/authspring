...
**Goal:** Add **`GET /api/verify-email/{id}/{hash}`** matching Laravel `routes/auth.php` (`verification.verify.api`) and `VerifyEmailController`: validate Laravel **signed URL** + **SHA-1 email hash**, set `email_verified_at` when needed, then **HTTP 302** redirect to the SPA with `email_verified`, JWT `api_token`, `auto_login`, `user_id`, `user_name` (Sanctum replaced by existing **`JwtService`**).

**Architecture:** A small **`LaravelSignedUrlValidator`** ports `Illuminate\Routing\UrlGenerator::hasCorrectSignature` + `signatureHasNotExpired` (HMAC-SHA256 over `request URL without query` + `?` + query string **excluding** `signature`). **`EmailVerificationService`** loads `User` by id, checks path `hash` equals **SHA-1 hex** of the user email (same as `Illuminate\Foundation\Auth\EmailVerificationRequest::authorize`). **`VerifyEmailController`** returns **`RedirectView`** or `ResponseEntity` with `Location` built from **`FrontendProperties.baseUrl`**. Signing key **must match Laravel `APP_KEY`** (decoded) when links are generated by Laravel; document env **`APP_URL`** / host alignment so the signed URL string matches what Laravel hashed.

**Tech stack:** Spring Boot 4, Spring MVC (`API-Version: 1`), `javax.crypto.Mac` (HMAC-SHA256), `MessageDigest` (SHA-1), existing JPA `User` / `UserRepository`, existing `JwtService`, MockMvc + Testcontainers ITs.
...
- Email verification authorize: `vendor/laravel/framework/src/Illuminate/Foundation/Auth/EmailVerificationRequest.php` — `hash_equals` on id and `sha1(email)` vs `hash`.
...
| File | Responsibility |
|------|----------------|
| `app/src/main/java/com/authspring/api/config/VerificationProperties.java` | `app.verification.signing-key` (raw bytes or string; must match Laravel `APP_KEY` used for signed routes). |
...
| `app/src/main/java/com/authspring/api/web/VerifyEmailController.java` | `GET /api/verify-email/{id}/{hash}` → redirect or 403. |
| `app/src/main/java/com/authspring/api/security/SecurityConfig.java` | `permitAll` for `GET /api/verify-email/**`. |
...
- `requestURL` = `https://example.com/api/verify-email/1/deadbeef...`
...
- Method: `EmailVerificationOutcome verify(HttpServletRequest request, Long id, String hash)`:
  ...
  4. Compute `sha1Hex(user.getEmail())` — must match Laravel: `sha1($email)` on **verification email**; Laravel uses `getEmailForVerification()` which is usually the same string as stored email. Normalize: use **exact** DB email bytes; if Laravel lowercases, use `Locale.ROOT` lowercase for SHA-1 input to match.
  5. If `!constantTimeEquals(sha1Hex, hash)` → `InvalidOrExpiredLink()`.
...
```
  ...
  9. Return `new RedirectToFrontend(user, url)`.
...
git commit -m "feat: email verification service (signed URL + SHA-1 hash)"
```
...
git commit -m "feat: GET /api/verify-email/{id}/{hash} with redirect"
```
...
2. Compute `hash = sha1Hex(email)` matching service.
...
4. `mockMvc.perform(get("/api/verify-email/{id}/{hash}").queryParam("expires", ...).queryParam("signature", ...).header("API-Version", "1"))`  
...
| Requirement | Task |
|-------------|------|
| Route `GET /api/verify-email/{id}/{hash}` (under `api` prefix like Laravel `routes/api.php`) | Task 4 |
| Signed URL (`signature`, `expires`) | Tasks 2–3 |
| SHA-1 `hash` vs email | Task 3 |
